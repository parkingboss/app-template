...
async function replaceReferencesInDir(replacements) {
    ...
    await Promise.all(
        files
            .filter((f) => path.extname(f).match(/\.js|\.css|\.html|\.map/gi))
            .map((f) => replaceReferencesInFile(f, replacements))
    );
}
...
async function hash(oldPath) {
    ...
    await Promise.all([fs.rename(oldPath, newPath), fs.rename(oldPath + '.map', newPath + '.map')]);

    return [oldName, newName];
}

async function hashBuild() {
    const replacements = await Promise.all([hash(oldScriptFile), hash(oldStyleFile)]);
